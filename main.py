...
from bot.handlers.del_all import register_dell_all
from bot.handlers.del_one import register_dell_one
...
def register_all_handlers(dp):
    ...
    register_get(dp)
    register_help(dp)
    register_dell_all(dp)
    register_dell_one(dp)
...
menu = ReplyKeyboardMarkup(resize_keyboard=True).add(button1).add(button2).add(button3).add(button4).add(button5).add(button6).add(button7)
